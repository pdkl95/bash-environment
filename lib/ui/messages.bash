...
_debug_msg() {
    ...
    while (( $# > 0 )) ; do
        case "$1" in
            ...
            --)           shift ; break ;;
            -*)           _derr "Unknown option to _debug_msg: $1" ; return ;;
            *)            TXT="${TXT} $1" ; shift ;;
        esac
    done
    #echo ${_PR}
...
echo_debug() {
    if (( ${bashEV[VERBOSE]} > 2 )) ; then
        _debug_msg --stderr --name DEBUG --id D --marker '~' --color 36 "${FUNCNAME[1]}" "$@"
    fi
}
...
echo_info() {
    if (( ${bashEV[VERBOSE]} > 1 )) ; then
        _debug_msg --stderr --name INFO  --id I --marker '-' --color 32 "${FUNCNAME[1]}" "$@"
    fi
}

echo_error() {
    if (( ${bashEV[VERBOSE]} > 0 )) ; then
        _debug_msg --stdout --name ERROR --id E --marker '*' --color 31 "${FUNCNAME[1]}" "$@"
    fi
}

echo_and_run() {
    if (( ${bashEV[VERBOSE]} > 1 )) ; then
        _debug_msg --stdout --name ERRORC --id X --marker '!' --color 35 "${FUNCNAME[1]}" "$@"
    fi
    "$@"
}
